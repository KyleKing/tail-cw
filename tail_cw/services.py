"""Wires the Textual shell to real AWS calls.

Argument parsing lives in :mod:`tail_cw.parser` and the export pipelines in
:mod:`tail_cw.cli`; this module fills :class:`ShellServices` with the live
pipelines and hands the parsed arguments to the dispatcher. It is imported only
once a command has been chosen, because it pulls in aiobotocore, Polars, and
Textual (:mod:`tail_cw.__main__` records the measurement).
"""
...
def run(args: argparse.Namespace, parser: argparse.ArgumentParser) -> int:
    """Run the command the parsed arguments name, against real AWS."""
    return dispatch(args, parser, _run_shell)
