# ruff: file-ignore[unused-async] - a fake service matches an awaitable signature without awaiting
...
from tail_cw.query.rollup import Granularity, roll_up
...
def _services(**overrides) -> ShellServices:
    async def roll_up_logs(groups: Sequence[str], start: datetime, end: datetime) -> object:
        del groups
    ...
    async def list_alarms(start: datetime, end: datetime) -> tuple[list[AlarmSummary], dict[str, int]]:
        del start, end
        return [_alarm('quiet-alarm'), _alarm('flapping-alarm')], {'flapping-alarm': 52, 'quiet-alarm': 1}

    async def run_insights(groups: Sequence[str], query: str, start: datetime, end: datetime) -> InsightsResult:
        del groups, query, start, end
    ...
    defaults = {'roll_up_logs': roll_up_logs, 'list_alarms': list_alarms, 'run_insights': run_insights}
...
@asynccontextmanager
async def _report(app: TailCWApp) -> AsyncIterator[Pilot[None]]:
    """Run the app until its report has finished loading.
...
async def test_alarm_transitions_are_read_over_a_useful_lookback():
    ...
    app = _app(ReportKind.ALARMS, services=_services(list_alarms=list_alarms))
